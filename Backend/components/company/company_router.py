...
from fastapi import APIRouter, FastAPI, Form, Request, Depends, HTTPException, UploadFile, File
...
class CompanyEditResponse(BaseModel):
    id: int
    name: str
    slogan: str
    description: str
...
@router.get("/api/company/{company_id}/edit", response_model=CompanyEditResponse)
async def get_company_for_edit(
    company_id: int, 
    user: dict = Depends(get_current_user)
):
    """Get company data for editing (API for React)"""
    conn = get_db_connection()
    cursor = get_db_cursor(conn)
    try:
        cursor.execute(
        ...
        company_dict = {
            ...
            "name": company[1],
            "slogan": company[2] or "",
            "description": company[3] or "",
            "website": company[4] or "",
            "phone_number": company[5] or "",
            "products": company[6] or "",
            "services": company[7] or "",
            "marketing_goals": company[8] or "",
            "target_age_groups": company[9] or "",
            "target_audience_types": company[10] or "",
            "target_business_types": company[11] or "",
            "target_geographics": company[12] or "",
            "preferred_platforms": company[13] or "",
            "special_events": company[14] or "",
            "brand_tone": company[15] or "",
            "monthly_budget": company[16] or "",
            "marketing_challenges": company[17] or "",
            "logo_url": company[18]
        }
...
@router.put("/api/company/{company_id}")
async def update_company_api(
    ...
    """Update company via API (for React)"""
    conn = get_db_connection()
    cursor = get_db_cursor(conn)
    try:
        # Validate website format
        if company_data.website and not validate_website(company_data.website):
            raise HTTPException(status_code=400, detail="Invalid website URL format")
        
        # Validate phone number format
        if company_data.phone_number and not validate_phone(company_data.phone_number):
            raise HTTPException(status_code=400, detail="Invalid phone number format")
        else:
            company_data.phone_number = re.sub(r'[^\d+]', '', company_data.phone_number)
        ...
        if logo and logo.filename:
            ...
            logo_url = result[0] if result else None
        ...
        cursor.execute(
            ...
            (
                ...
                ",".join(company_data.marketing_goals),
                ",".join(company_data.target_age_groups),
                ",".join(company_data.target_audience_types),
                ",".join(company_data.target_business_types),
                ",".join(company_data.target_geographics),
                ",".join(company_data.preferred_platforms),
                ",".join(company_data.special_events),
                company_data.brand_tone,
                company_data.monthly_budget,
                ",".join(company_data.marketing_challenges),
                logo_url,
                company_id,
                user["user_id"]
            )
        ...
        return JSONResponse(
            status_code=200,
            content={"success": True, "message": "Company updated successfully"}
        )
